...
Date: 2026-08-30 Status: approved test-only scope for GitHub issue #23

## Goal

Measure the real App Server behavior for a synthetic cache root outside the target repository and `/tmp` allowance.
...
The test creates owner-controlled synthetic `cache`, `home`, and `sibling` roots under `/Users/Shared`.
