...
Show a bounded list of affected repository-relative paths for a dirty
worktree preflight refusal.
...
If the two observations disagree on clean or dirty, it throws
`GIT_SNAPSHOT_RACE`.
...
The implementation changes `src/runtime/git.ts`, its preflight call sites,
and `test/integration/git.test.mjs`.
The call sites await the existing clean-worktree assertion.
It does not change clean-worktree policy, Git process limits, dependencies,
README copy, GitHub state, or releases.
...
Oracle found no project precedent for separating snapshot and diagnostic
status observations.
