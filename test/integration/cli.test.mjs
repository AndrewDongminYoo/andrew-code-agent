...
test("compiled CLI exposes exact help and routes the six-command grammar", async () => {
  ...
  assert.deepEqual(calls, [["commit", "long"], ["commit", "long"], ["commit", "short"], ["doctor"], ["review", undefined], ["review", "origin/develop"], ["run", "/repo", "prompt"], ["resume", "thread-1", undefined], ["resume", "thread-1", "next"], ["status", undefined], ["status", "thread-1"]]);
});

test("command help exits before handlers and invalid grammar exits 2", async () => {
  const { cliModule } = modules();
  const calls = [];
  const handlers = Object.fromEntries(["commit", "doctor", "review", "run", "resume", "status"].map((name) => [name, async () => { calls.push(name); return 0; }]));
  const topLevelUsage = "Usage:\nandrew-agent commit [--long | --short]\nandrew-agent doctor\nandrew-agent review [base]\nandrew-agent run <repository> <prompt>\nandrew-agent resume <thread-id> [prompt]\nandrew-agent status [thread-id]\n";
  for (const [argv, usage] of [[["commit", "-h"], "andrew-agent commit [--long | --short]"], [["doctor", "-h"], "andrew-agent doctor"], [["review", "-h"], "andrew-agent review [base]"], [["run", "--help"], "andrew-agent run <repository> <prompt>"], [["resume", "-h"], "andrew-agent resume <thread-id> [prompt]"], [["status", "--help"], "andrew-agent status [thread-id]"]]) {
    const output = capture();
