...
test("compiled CLI exposes exact help and routes the seven-command grammar", async () => {
  const { cliModule } = modules();
  const output = capture();
  const calls = [];
  const handlers = { commit: async (_io, format) => { calls.push(["commit", format]); return 0; }, doctor: async () => { calls.push(["doctor"]); return 0; }, pr: async (...args) => { calls.push(["pr", ...args.slice(0, 1)]); return 0; }, review: async (...args) => { calls.push(["review", ...args.slice(0, 1)]); return 0; }, run: async (...args) => { calls.push(["run", ...args.slice(0, 2)]); return 0; }, resume: async (...args) => { calls.push(["resume", ...args.slice(0, 2)]); return 0; }, status: async (...args) => { calls.push(["status", ...args.slice(0, 1)]); return 0; } };
  assert.equal(await cliModule.main(["--help"], { ...output, handlers }), 0);
  assert.equal(output.output().stdout, "Usage:\nandrew-agent commit [--long | --short]\nandrew-agent doctor\nandrew-agent pr [base]\nandrew-agent review [base]\nandrew-agent run <repository> <prompt>\nandrew-agent resume <thread-id> [prompt]\nandrew-agent status [thread-id]\n");
  ...
  assert.equal(await cliModule.main(["pr"], { ...capture(), handlers }), 0);
  assert.equal(await cliModule.main(["pr", "origin/develop"], { ...capture(), handlers }), 0);
  assert.equal(await cliModule.main(["review"], { ...capture(), handlers }), 0);
  assert.equal(await cliModule.main(["review", "origin/develop"], { ...capture(), handlers }), 0);
  ...
  assert.deepEqual(calls, [["commit", "long"], ["commit", "long"], ["commit", "short"], ["doctor"], ["pr", undefined], ["pr", "origin/develop"], ["review", undefined], ["review", "origin/develop"], ["run", "/repo", "prompt"], ["resume", "thread-1", undefined], ["resume", "thread-1", "next"], ["status", undefined], ["status", "thread-1"]]);
});

test("command help exits before handlers and invalid grammar exits 2", async () => {
  const { cliModule } = modules();
  const calls = [];
  const handlers = Object.fromEntries(["commit", "doctor", "pr", "review", "run", "resume", "status"].map((name) => [name, async () => { calls.push(name); return 0; }]));
  const topLevelUsage = "Usage:\nandrew-agent commit [--long | --short]\nandrew-agent doctor\nandrew-agent pr [base]\nandrew-agent review [base]\nandrew-agent run <repository> <prompt>\nandrew-agent resume <thread-id> [prompt]\nandrew-agent status [thread-id]\n";
  for (const [argv, usage] of [[["commit", "-h"], "andrew-agent commit [--long | --short]"], [["doctor", "-h"], "andrew-agent doctor"], [["pr", "-h"], "andrew-agent pr [base]"], [["review", "-h"], "andrew-agent review [base]"], [["run", "--help"], "andrew-agent run <repository> <prompt>"], [["resume", "-h"], "andrew-agent resume <thread-id> [prompt]"], [["status", "--help"], "andrew-agent status [thread-id]"]]) {
    const output = capture();
  ...
  for (const [argv, expected] of [
    ...
    [["pr", "one", "two"], "Invalid command usage.\nUsage: andrew-agent pr [base]\n"],
    [["pr", "--long"], "Invalid command usage.\nUsage: andrew-agent pr [base]\n"],
    [["review", "one", "two"], "Invalid command usage.\nUsage: andrew-agent review [base]\n"],
    [["review", "--long"], "Invalid command usage.\nUsage: andrew-agent review [base]\n"],
